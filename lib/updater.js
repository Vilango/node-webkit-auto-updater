        ...
        if (self.existsUrlForPlatform(platform)) {
          ...
          self.isFileUrl( curUrl, function(err, isFile){
            ...
            if (isFile) {
                ...
                self.mount(tmpfilepath, tmpId, function(err, mpoint) {
                  ...
                  self.checkNewApp( newAppPath, function(error, exists){
                    ...
                    if (exists) {
                      ...
                      errors.push(new UpdaterError("App is not in the downloaded file", newAppPath));
                    }
                  });
                });
              });
              ...
              errors.push(new UpdaterError("Url is not a file URL: "+curUrl));
            }
          });
          // } catch (error) {
          ...
          //     errors.push(new UpdaterError("Something went totally wrong when doing the cleanup", err)); 
          //   }
          // }
        } else {
          logger.info("We do not have a download link for the current platform", platform, this.config.sources);
          errors.push(new UpdaterError("No update found for platform: "+platform));
        }
    }
};