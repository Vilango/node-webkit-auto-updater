...
function run(path, args, options, cb){
  ...
  // FIXME: don't publish sp - testing only!!!
...
Updater.prototype = {
    ...
    startup: function (destAppPath) {
      if(destAppPath){
        console.log("Updater.init", "we are in UPDATE MODE", Updater.CONST.MODE_APPLYVERSIONUPDATE);
        process.stdout.write(Updater.CONST.MODE_APPLYVERSIONUPDATE+'\n');
        //process.disconnect();
      } else {
        console.log("Updater.init", "we are in", Updater.CONST.MODE_NORMAL);
        process.stdout.write(Updater.CONST.MODE_NORMAL+'\n');
        //process.disconnect();
      }
    },
    ...
    getUrlForPlatfrom: function(myPlatform){
        ...
        if ( purl && _.isString(purl) && (_str.startsWith(purl, "http") || _str.startsWith(purl, "file")) )
            return purl;
        else
            return "";
    },
    ...
    findMountPoint: function(dmg, callback) {
        exec('hdiutil info -plist', function(error, stdout, stderr){
          if (!error) {
            ...
            logger.error("Error getting hdiutil info for", dmg, error);
    ...
    mount: function(dmg, mountpointId, callback) {
        ...
        this.findMountPoint(dmg, function(err, found_mount_point){
          if (!err) {
            ...
            logger.error("mount cb findMountPoint error", mountpointId);
    ...
    applyVersionUpdateStep1: function(tempAppPath, cb){
      ...
      logger.info("applyVersionUpdateStep1", tempAppPath, appPath, appExec);
